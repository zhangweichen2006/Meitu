...
def supervised_training_iter(
    ...
    semantic_scale=10.0, detail_scale=10.0, matte_scale=10.0):
    ...
    matte_l1_loss2 = smooth_l1_loss(pred_matte, gt_matte) + 4.0 * smooth_l1_loss(pred_boundary_matte, gt_matte)
    matte_compositional_loss2 = smooth_l1_loss(image * pred_matte, image * gt_matte) \
                               + 4.0 * smooth_l1_loss(image * pred_boundary_matte, image * gt_matte)
    matte_loss2 = torch.mean(matte_l1_loss2 + matte_compositional_loss2)
    matte_loss2 = matte_scale * matte_loss2

    matte_l1_loss3 = fea_loss(pred_matte, gt_matte) + 4.0 * fea_loss(pred_boundary_matte, gt_matte)
    matte_compositional_loss3 = fea_loss(image * pred_matte, image * gt_matte) \
                                + 4.0 * fea_loss(image * pred_boundary_matte, image * gt_matte)
    matte_loss3 = torch.mean(matte_l1_loss3 + matte_compositional_loss3)
    matte_loss3 = matte_scale * matte_loss3

    # kl_loss(F.log_softmax(df, dim=1), F.softmax(fs_i, dim=1))
    matte_l1_loss4 = kl_loss(F.log_softmax(pred_matte, dim=1), F.softmax( gt_matte)) +\
                      4.0 * kl_loss(F.log_softmax(pred_boundary_matte, dim=1), F.softmax( gt_matte))
    matte_compositional_loss4 =kl_loss(F.log_softmax(image * pred_matte, dim=1), F.softmax(image * gt_matte)) \
                                + 4.0 * kl_loss(F.log_softmax(image * pred_boundary_matte, dim=1), F.softmax( image * gt_matte))
    matte_loss4 = torch.mean(matte_l1_loss4 + matte_compositional_loss4)
    matte_loss4 = matte_scale * matte_loss4




    # calculate the final loss, backward the loss, and update the model 
    loss = semantic_loss + matte_loss+matte_loss2+matte_loss3+matte_loss4
    loss.backward()
    optimizer.step()

    # for test
    return semantic_loss,  matte_loss
