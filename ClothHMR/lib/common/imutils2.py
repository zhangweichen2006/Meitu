...
def load_img(img_file):
    ...
    return torch.tensor(img).permute(2, 0, 1).unsqueeze(0).float(),img, img.shape[:2]
def load_imgs(img_file):
    image_dataset = Image.open(
        img_file).convert(
        'RGB')
    ...
    mask_dataset = Image.open(
        img_file[:-4]+"_mask.png")
    mask_dataset = np.array(mask_dataset)
    mask_dataset = (mask_dataset - 1) / 254
    # mask_dataset = torch.from_numpy(mask_dataset).cuda().float()
    # mask_dataset = mask_dataset.unsqueeze(0)
...
def process_image(img_file, hps_type, single, input_res, detector):
    ...
    img_raw, img_ori,(in_height, in_width) = load_img(img_file)
    tgt_res = input_res * 2
    ...
    for idx in range(len(boxes)):
        ...
        #     padding_mode='zeros',
        #     align_corners=True
        # ).squeeze(0).permute(1, 2, 0).numpy().astype(np.uint8) #512,512,4
        ...
        img_rembg = remove(img_ori, post_process_mask=True, session=new_session("u2net"))
        img_mask = remove_floats(img_rembg[:, :, [3]])

        # sapiens
        # mask_file= cv2.imread(img_file.replace("smpl_iamge",""))
        # mask=cv2.cvtColor(mask_file, cv2.COLOR_BGR2GRAY)
        # _,img_mask= cv2.threshold(mask, 0, 255, cv2.THRESH_BINARY)
        # print(img_rembg.shape)
        ...
        # img_hps = transform_to_tensor(224, constants.IMG_NORM_MEAN,
        #                               constants.IMG_NORM_STD)(Image.fromarray(img_ori))

        img_token = transform_to_tensor(256, constants.IMG_NORM_MEAN,
                                        constants.IMG_NORM_STD)(Image.fromarray(img_np))
        ...
        # img_ori = (img_ori / 255 - 0.5) * 2
        # img_ori = torch.from_numpy(img_ori).cuda().permute(2, 0, 1).float()
        ...
        img_tokenhmr_list.append(img_token)
        # img_ori_lst.append(img_ori)
        # mask_ori_lst.append(mask_ori)
    ...
    return_dict = {
        ...
        "img_token": torch.stack(img_tokenhmr_list).float(),
        ...
        # "img_ori": torch.stack(img_ori_lst).float(),
        # "mask_ori": torch.stack(mask_ori_lst).float()
    }
