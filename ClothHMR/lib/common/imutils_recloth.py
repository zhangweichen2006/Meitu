...
from remcloth.test_single_fun import re_colth_np
...
def process_image(img_file, hps_type, single, input_res, detector):
    ...
    for idx in range(len(boxes)):
        ...
        img_mask2=img_mask*255
        temp=np.zeros_like(img_mask)
        temp[img_mask2>10]=1
        ...
        re_colth,mask_re_cloth = re_colth_np(Image.fromarray(img_np))
        img_np=re_colth.astype(np.uint8)
        img_mask=np.logical_and(temp, mask_re_cloth[:,:,None])

        image_save = img_mask[:,:,0].copy()*255
        image_save = Image.fromarray(image_save.astype(np.uint8))
        # image_save.save("/media/star/Extreme SSD/code/VS/mask.png")
