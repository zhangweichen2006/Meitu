...
class TestDataset:
    def __init__(self, cfg, device):
        ...
        if self.hps_type == "pymafx":
            ...
            self.hps2, model_cfg2= load_tokenhmr(
                checkpoint_path="/media/star/Extreme SSD/code/VS/tokenhmr/data/checkpoints/tokenhmr_model_latest.ckpt", \
            ...
            self.hps2 = self.hps2.to(device)
            self.hps2.eval()
            self.smpl_model = self.hps2.smpl
            #
    ...
    def __getitem__(self, index):
        ...
        with torch.no_grad():
            if self.hps_type == "pixie":
                ...
                smplx_mesh=trimesh.load_mesh("/media/star/Extreme SSD/code/VS/vm_smplx.obj")

                output=smplx2smpl(torch.from_numpy(np.array(smplx_mesh.vertices)).unsqueeze(0).to(self.device).float(),
                                  torch.from_numpy(self.smpl_faces.astype(np.int32)).unsqueeze(0).to(self.device),
        ...
        if self.hps_type == "pymafx":
            ...
            trans = torch.tensor(np.array([[0,0,0]])).to(self.device)
            scale=torch.tensor(np.array([1])).to(self.device)

            arr_dict["scale"] = scale.unsqueeze(1).to(self.device).float()
            arr_dict["trans"] = trans.unsqueeze(1).to(self.device).float()
