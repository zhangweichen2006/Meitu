...
def draw_text(input_image, content):
    ...
    for key in sorted(content.keys()):
        ...
        cv2.putText(image, text, (int(start_x), int(start_y)), 0, 0.45, (0, 0, 255))
        start_y += margin
...
class Renderer(object):
    ...
    def __init__(self, width=800, height=600, near=0.5, far=1000, faces=None):
        self.colors = {'pink': [.9, .7, .7], 'light_blue': [0.65098039, 0.74117647, 0.85882353],'green':[0.69,0.93,0.93] }
        self.width = width
        self.height = height
        self.faces = faces
