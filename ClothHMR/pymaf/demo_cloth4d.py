...
def run_image_demo(args):
    ...
    smpl_vertex_code, smpl_face_code, smpl_faces, smpl_tetras = \
        util.read_smpl_constants('/media/star/2023/CODE/GYQ/SPIN-master/data')
    ...
    for img_path in sorted(glob.glob("/media/star/Extreme SSD/cloth4d/CLOTH4Dsub_8views_part1/*/*/render/*.png")):
        path, filename = os.path.split(img_path)
        os.makedirs(path.replace("render", "pymaf").replace("cloth4d", "cloth4d_metric"), exist_ok=True)
        # for img_path in glob.glob("/media/gpu/软件/GYQ/dataset/body_metric/pic/female/*.jpg"):
        print(img_path)
    ...
    # Preprocess input image and generate predictions
        ...
        init_smpl_fname =  os.path.join(output_path, img_name+".obj")
        exit()
        obj_io.save_obj_data({'v': (pred_vertices), 'f': smpl_faces},
                             img_path.replace("render","pymaf").replace("cloth4d","cloth4d_metric")[:-4]+'.obj')

        # cv2.imwrite(save_name + '_smpl.png', img_shape)
        # cv2.imwrite(save_name + '_smpl_side.png', img_shape_side)

        print(f'Saved the result image to {init_smpl_fname }.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_folder', type=str, default="/media/star/2023/CODE/GYQ/PyMAF-smpl/examples/test",
                        help='input image folder')
    parser.add_argument('--output_folder', type=str, default="/media/star/2023/CODE/GYQ/PyMAF-smpl/examples/test",
    ...
    parser.add_argument('--cfg_file', type=str, default='/media/bbnc/FE345AE3345A9F09/loose_cloth/clothHMR/pymaf/configs/pymaf_config.yaml',
                        help='config file path.')
    parser.add_argument('--checkpoint', default="/media/star/2023/CODE/GYQ/PyMAF-smpl/models/PyMAF_model_checkpoint.pt",
    ...
    parse_args(args)
    # for i in glob.glob(os.path.join(args.image_folder,"*.jpg")):
    run_image_demo(args)
