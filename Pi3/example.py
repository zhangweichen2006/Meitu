        ...
        if pts_world.numel() > 0:
            ...
            if in_bounds.any():
                ...
                try:
                    ...
                    for axis, angle, suffix in rotations:
                        rotated_world2cam = create_rotated_camera(axis, angle)
                        tile_paths.append(render_with_world2cam_suffix(rotated_world2cam, '6views', suffix, pts_world, cols, H, W))
                    # Compose a 2x3 grid of the six rotated views
                    try:
                        tiles = []
                        for p in tile_paths:
                        ...
                        for idx, tile in enumerate(tiles):
                            ...
                            c = idx % 3
                            grid.paste(tile, (c * W, r * H))
                        base_noext = png_path.rsplit('/', 1)[0] + '/6views/' + png_path.rsplit('/', 1)[1].rsplit('.', 1)[0]
                        grid_path = base_noext + '.png'
                        grid.save(grid_path)
                        print(f"Saved 2x3 grid of six views to: {grid_path}")
